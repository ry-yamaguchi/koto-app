...
//
// ── B'-3d-2a: renderer から shared へ移した ──────────────────────────────
// executeTool 本体（toolExecCore.ts）が main プロセスでも動く形（io ports 方式）に
// なるため、その本体が使う純関数はここに置く。renderer/editFile.ts は re-export だけ残す
// （呼び出し側を壊さない・B'-3d-1a の usage.ts と同じ作法）。
