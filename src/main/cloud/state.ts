...
export type EnvMeta = {
  ...
  imageTag?: string
  /**
   * レジストリの権限を**一時的に上げたまま**になっている印（2026-08-20 点検）。
   *
   * 片づけの前に上げ、終わったら戻す。**その途中でアプリが強制終了すると
   * 上がったまま残る**ので、印を残しておき、次に片づけを始めるときに戻す。
   */
  registryElevatedAt?: string
  /**
   * レジストリ作成時に添えた分類（Description/Tags）が、実際に反映されることを
   * **確かめられたか**（roadmap #36）。
   *
   * ── なぜ要るか ────────────────────────────────────────────────────────
   * 「作るときに Description/Tags を設定できるか」は未確認（掟1）。作成のたびに
   * 試して失敗を繰り返すのは無駄なので、一度分かったら次回以降はその前提で振る舞う
   * （試すかどうかを決める・毎回は試さない）。
   *
   * - `true`: 作成後に読み直して、実際に反映されていることを確認できた
   * - `false`: 分類を付けずに作成した（未対応と分かった／作成が失敗して外した）
   * - 未設定（キー無し）: まだ分からない（次のレジストリ作成でまた試す）
   *
   * **既存のレジストリに後から付けにいくことはしない**（実在資源の変更・roadmap #36
   * 「やらないこと」）。この値は「次に新規作成するとき、どう振る舞うか」だけに使う。
   */
  registryMetaSupported?: boolean
}
