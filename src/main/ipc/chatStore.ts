...
//
// アトミック書き込み（.tmp に書いてから rename）で、保存中のクラッシュによるファイル破損を防ぐ。
...
export function registerChatStoreHandlers(_deps: IpcDeps) {
  ...
  ipcMain.handle('chat:loadProject', (_, projectDir: string) => {
    try {
      return { ok: true, json: readFileOrNull(projectChatPath(projectDir)) }
    } catch (e: any) {
  ...
  ipcMain.handle('chat:saveProject', (_, projectDir: string, json: string) => {
    try {
      ...
      atomicWriteFileSync(projectChatPath(projectDir), json)
      return { ok: true }
    } catch (e: any) {
