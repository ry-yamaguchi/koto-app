...
contextBridge.exposeInMainWorld('electronAPI', {
  fs: {
    ...
    /** 未使用ファイルを調べる（**何も変えない**。Node/PHP にも対応。roadmap #18・#22）。 */
  ...
  // src/main/chat/approvalStore.ts（メモリのみ・永続化しない）。要否判定・文面組み立ては
  // main（turnRunner.ts）が src/shared/approvalPlan.ts の純関数で行い、renderer は
  // 「渡された文面のダイアログを出して答えるだけ」の純UI（ChatPanel.tsx）になる。
  approval: {
    /** 承認待ち一覧（画面が（再）起動したときの取りこぼし回収＝駐機の再提示に使う）。 */
    list: () => ipcRenderer.invoke('approval:list'),
    /** 許可=true／拒否=false で回答する。知らない id・二重回答は false が返る（無視）。 */
    answer: (id: string, approved: boolean) => ipcRenderer.invoke('approval:answer', id, approved),
    /** main が一覧を変えるたび届く通知（learning.onChanged と同じ作法・購読解除関数を返す）。 */
    onChanged: (cb: (list: { id: string; dir: string | null; label: string }[]) => void) => {
      const handler = (_: Electron.IpcRendererEvent, list: { id: string; dir: string | null; label: string }[]) => cb(list)
      ipcRenderer.on('approval:changed', handler)
      return () => ipcRenderer.removeListener('approval:changed', handler)
    },
  },
