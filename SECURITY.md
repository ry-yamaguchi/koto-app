...
- **🛡 簡易セキュリティチェックは、AI による読解であって、静的解析ツールでも脆弱性スキャナでもありません。** 既知の脆弱性データベースとの照合・依存ライブラリの検査・動的な攻撃の試行は行いません。**見逃しは設計上の既知の制約**です（画面にも「最終的にはご自身で確認してください」と表示しています）。チェックが特定の問題を見つけられなかったこと自体は、脆弱性の報告としては受け付けていません。なお、このチェックを実行すると**公開対象のコードがさくらのAI Engine へ送られます**（`.env` などの秘密ファイルは名前だけを伝え、中身は送りません）。
