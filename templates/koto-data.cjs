// koto-data.cjs — Koto が用意した「データの保存」（require を使うアプリ向け）。
//
// このファイルは Koto が置いたものです。**中身を書き換える必要はありません。**
// アプリからは下の4つだけを使ってください。
//
//   const { list, get, save, remove } = require('./koto-data.cjs')
//
...
//
// ── このファイルと koto-data.js の違い ────────────────────────────────
// 中身の働きはまったく同じで、読み込み方だけが違います。
// このアプリは `require` で読み込む形なので、こちらを置いています。
// **package.json の "type" を変えないでください。** 変えるとアプリ全体が
// `require` を使えなくなり、起動しなくなります。
//
// ── なぜこれを通すのか ────────────────────────────────────────────────
...
// ── 使う側へ公開する（koto-data.js と同じ4つ＋storageMode） ──────────────
module.exports = { list, get, save, remove, storageMode }
